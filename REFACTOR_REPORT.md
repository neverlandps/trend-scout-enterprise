...
| P1-7 | LLM 评分并发化 | ✅ | `asyncio.gather` + `Semaphore(5)` + 连续失败熔断；LLM 调用与写库解耦 |
| P1-8 | 连接池与健康检查 | ✅ | PostgreSQL pool_size/max_overflow/pre_ping；compose healthcheck |
| P1-9 | N+1 查询修复 | ✅ | 报告服务批量 Source 查询；trends evidence `joinedload` |
| P1-10 | 模型文件拆分 | ✅ | models 拆为 7 个模块，`models.py` 保留兼容导出层；schemas 17 处 `ConfigDict` 迁移 |
...
多 Agent 协作（CrewAI 角色分工）、事件总线、缓存层、容器镜像签名/SBOM、异常检测服务、LangGraph interrupt/resume（工作流挂起-恢复）。

### P2 已落地（2026-07-22 追加）

| 项目 | 状态 | 说明 |
|------|------|------|
| SAST/DAST CI | ✅ | `security.yml`：bandit（基线 0 High）+ pip-audit + trivy，2 周观察期后转强制 |
| 安全响应头 | ✅ | `SecurityHeadersMiddleware`：nosniff/X-Frame-Options/CSP（API 严格、HTML 宽松）/可选 HSTS |
| JWT RS256 + 轮换 | ✅ | kid 公钥环支持轮换窗口，HS256 向后兼容，`scripts/generate_jwt_keys.py` |
| 向量数据库/语义检索 | ✅ | `SignalEmbedding`（JSON 向量）+ `EmbeddingService` + similar/semantic-search API；pgvector 为演进路径 |
| LangGraph 工作流 | ✅ | `workflows/scan_graph.py` 显式状态机编排扫描全流程；interrupt/resume 已记录演进方案 |
