...
export function ReportsPage() {
  ...
  const handleCreate = async () => {
    try { await createReport({ title: title || undefined, item_ids: Array.from(selected) }); setTitle(''); setSelected(new Set()); load() } catch (e) { setError(String(e)) }
  }
  ...
  return (
    <Stack tokens={{ childrenGap: 16 }}>
      ...
      <Stack horizontal tokens={{ childrenGap: 8 }} verticalAlign="end">
        ...
        <PrimaryButton text="Generate PDF Report" onClick={handleCreate} disabled={selected.size === 0} />
      </Stack>
          ...
          {reports.map(r => (
            <Stack key={r.id} horizontal horizontalAlign="space-between" verticalAlign="center" styles={{ root: { border: '1px solid #e5e7eb', borderRadius: 8, padding: 12 } }}>
              <Stack tokens={{ childrenGap: 4 }}>
                <Text variant="mediumPlus">{r.title || 'Untitled Report'}</Text>
                <Text variant="small" styles={{ root: { color: 'gray' } }}>Status: {r.status} | Created: {new Date(r.created_at).toLocaleString()}</Text>
              </Stack>
              {r.status === 'completed' && <DefaultButton text="Download" href={`/api/v1/reports/${r.id}/download`} target="_blank" />}
            </Stack>
          ))}
